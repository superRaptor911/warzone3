...
npm run test:touch        # optional (~75s): drives the real client in headless Chrome over CDP — real touch
                          # points on the pads, a rotate-then-quit-then-rejoin lifecycle pass (which measures
                          # screen brightness, the only way those renderer bugs are visible), and a desktop
                          # regression pass. Skips (exit 0) with no Chrome.
