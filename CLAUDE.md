...
#                                            (?map=outbreak&zoom=28&grid=1 — the controls are URL params)
...
> Phases 1 and 2 are done: the pipeline, and the levels drawn as places on top of
> it (rotational Compound, apartment-block Outbreak, authored overheads). Phase 3
> — per-material ambient, drawing the overheads, and the x-ray silhouette that
> keeps them legal — is specified in **`tasks/WORLD-ART.md`** along with the design
> decisions behind all three. Read it before changing maps or lighting.
